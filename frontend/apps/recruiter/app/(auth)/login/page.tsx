...
export default function RecruiterLogin() {
  ...
  const [role, setRole] = useState("recruiter"); // Default to recruiter since we're in recruiter app
  ...
  const handleCompleteGoogle = async () => {
    try {
      ...
      if (data.user.role === "candidate") {
        const candidateUrl = process.env.NEXT_PUBLIC_CANDIDATE_URL || "http://localhost:3001";
        window.location.href = `${candidateUrl}/login?token=${data.access_token}&role=${data.user.role}`;
      } else {
        router.push("/");
      }
    } catch (error: any) {
      ...
      try {
        ...
        if (data.needs_completion) {
          ...
          if (userRole === "candidate") {
            const candidateUrl = process.env.NEXT_PUBLIC_CANDIDATE_URL || "http://localhost:3001";
            window.location.href = `${candidateUrl}/login?token=${data.access_token}&role=${userRole}`;
          } else {
            router.push("/");
          }
        }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await loginMutation.mutateAsync({ username: email, password });
